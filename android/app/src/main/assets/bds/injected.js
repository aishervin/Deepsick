...
`),startLine:f,endLine:c}),a>=r.length)break;a=Math.max(f,a-n)}return o}function L(e){return e?(String(e).toLowerCase().match(/[a-z0-9_\u015f\u00e7g\u00f6\u0131\u00fc]+/gi)||[]).filter(n=>n.length>=2&&!K.has(n)):[]}function Q(e,t,n=5){if(!e||!t||!t.length)return[];const r=[];for(const S of t)r.push(...V(S,800,5));if(r.length===0)return[];const o=L(e);if(o.length===0)return[];const a=r.length,u=r.map(S=>L(S.content)),s=u.map(S=>S.length),c=s.reduce((S,k)=>S+k,0)/a||1,h={};for(const S of o){h[S]=0;for(const k of u)k.includes(S)&&h[S]++}const m=1.2,E=.75,x=[];for(let S=0;S<a;S++){const k=r[S],g=u[S],w=s[S];let i=0;const l={};for(const d of g)l[d]=(l[d]||0)+1;for(const d of o){const y=l[d]||0;if(y===0)continue;const b=h[d]||0,R=Math.log(1+(a-b+.5)/(b+.5))*(y*(m+1))/(y+m*(1-E+E*(w/c)));i+=R}const p=String(k.fileName).toLowerCase();for(const d of o)p.includes(d)&&(i+=12);i>0&&x.push({...k,score:i})}return x.sort((S,k)=>k.score-S.score).slice(0,Math.max(1,n))}function Z(e,t="Project"){if(!e||!e.length)return"";let n=`<BDS:PROJECT_CONTEXT>
...
`,n+=`\`\`\`${o}
`,n+=r.content+`
`,n+="```\n\n"}return n+="</BDS:PROJECT_CONTEXT>",n}function I(e,t){var x,S,k;t.sessionUserMsgCounts||(t.sessionUserMsgCounts={});const n=ee(e),r=te(e);let o=1;n&&n.length>0?(o=n.filter(g=>{const w=String(g.role||g.author||"").toLowerCase();return w==="user"||w==="human"}).length,t.sessionUserMsgCounts[r]=o):typeof e.prompt=="string"&&(e.message_id===1||e.parent_message_id==null?o=1:o=(t.sessionUserMsgCounts[r]||0)+1,t.sessionUserMsgCounts[r]=o);let a=!1,u=null;if(n&&n.length>0){u=N(n)||n[n.length-1];const g=A(u);if(g){const w=B(g),i=ne(n,u);let l=!1;const p=t.config.systemPromptInjectionFrequency||"first";if(p==="always")l=!0;else if(p==="every_x"){const y=t.config.systemPromptInjectionInterval||3;(o-1)%y===0?l=!0:i||(l=!0)}else l=!i,(n.length>1||t.hasInjected&&t.hasInjected(r))&&(l=!1);const d=O(w,r,t,l,n,u);window.dispatchEvent(new CustomEvent("bds:mutation-applied",{detail:JSON.stringify({conversationId:r,injectedText:d||"",userPrompt:w})})),d?(C(u,`${d}
...
`);return`<Deepsick> <BDS:SKILLS fingerprint="${_(e.config.skills)}">
${t}
</BDS:SKILLS> </Deepsick>`}function _(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.name}:${(t.content||"").length}`).sort().join("|")}function se(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.serverName}:${t.toolName}:${JSON.stringify(t.inputSchema||{})}`).sort().join("|")}function ae(e){if(!Array.isArray(e))return null;for(let t=e.length-1;t>=0;t--){const n=e[t];if(!n||typeof n!="object")continue;const r=String(n.role||n.author||"").toLowerCase();if(!(r==="user"||r==="human")&&(r==="assistant"||r==="ai"||r==="bot"))return n}return null}function P(e){return!e||typeof e!="string"?[]:e.split(new RegExp("[_-]|\\s+|(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])")).map(t=>t.toLowerCase().replace(/[^a-z0-9]/g,"")).filter(t=>t.length>0)}function ce(e,t){if(!e.length||!t.length)return 0;const n=new Set(t);let r=0;for(const o of e)n.has(o)&&r++;return r/e.length}function le(e,t){return t===1?e>=1:e>=.5}function ue(e,t,n){if(t.config.disableMemory||!t.config.memories.length)return"";const r=n?ae(n):null,o=r?A(r):"",a=[e,o].filter(Boolean).join(" "),u=P(a),s=[];for(const c of t.config.memories){if(c.importance==="always"){s.push(c);continue}if(!c.key)continue;const h=P(c.key);if(!h.length){a.toLowerCase().includes(c.key.toLowerCase())&&s.push(c);continue}const m=[...new Set(h)],E=ce(m,u);(le(E,m.length)||a.toLowerCase().includes(c.key.toLowerCase()))&&s.push(c)}return s.length?`<Deepsick>
${s.map(c=>`<BDS:memory_calls importance="${c.importance}">${c.key}: ${de(c.value)}</BDS:memory_calls>`).join(`
`)}
</Deepsick>`:""}function de(e){return String(e).replace(/<\//g,"<\\/").trim()}function he(e){const t=e.config&&e.config.activeProject;if(!t)return"";let n="";return t.instructions&&t.instructions.trim()&&(n+=t.instructions.trim()+`
`),`<Deepsick>
<BDS:PROJECT name="${t.name}">
${n}</BDS:PROJECT>
</Deepsick>`}function fe(e){const t=e.config.activeCharacter;if(!t||!t.content)return"";let n=`Character Name: ${t.name}
`;return t.usage&&(n+=`Usage Domain: ${t.usage}
`),n+=`---
${t.content.trim()}`,`<Deepsick> <BDS:RP>
${n}
</BDS:RP> </Deepsick>`}function pe(e){const t=[];if(e.config.injectSystemDateTime!==!1){const r=new Date;t.push(`User's System Date & Time: ${r.toLocaleString()}`)}const n=e.config.preferredLang;return n&&n.trim()&&t.push(`Always respond in ${n.trim()}.`),t.length===0?"":`<Deepsick>
${t.join(`
`)}
</Deepsick>`}function me(e,t){var w;const n=(w=e.config)==null?void 0:w.mcpToolSchemas;if(!Array.isArray(n)||!n.length)return"";const r=Number(e.config.mcpInlineMaxChars)||8e3,o=n.length,a=[`<Deepsick> <BDS:MCP fingerprint="${t}">`,"You have access to the following MCP (Model Context Protocol) tools via remote servers.",`To invoke them, use: <BDS:AUTO:MCP url="SERVER_NAME_OR_URL" tool="TOOL_NAME" args='{"key":"value"}'>`,"The extension will call the tool and inject the result.","Important: Only ONE tool per response. Wait for the result before invoking another. Never invoke multiple tools at the same time.","","Available tools:"].join(`
`),u="</BDS:MCP> </Deepsick>",s=n.map(i=>{let l=`- Server: ${i.serverName} (${i.serverUrl||i.serverName}) | Tool: ${i.toolName}`;if(i.description&&(l+=` | Description: ${i.description}`),i.inputSchema&&typeof i.inputSchema=="object"){const p=i.inputSchema.properties;if(p){const d=Object.entries(p).map(([y,b])=>{const T=(i.inputSchema.required||[]).includes(y)?" (required)":"";return`${y}: ${(b==null?void 0:b.type)||"any"}${T}`});d.length&&(l+=` | Params: ${d.join(", ")}`)}}return l}),f=[a,...s,u].join(`
`);if(f.length<=r)return f;const c=i=>`
... and ${i} more tool(s) not shown (MCP tool list exceeds inline character limit \u2014 all tools are still available for invocation).`,h=c(1),m=a.length+1+u.length+h.length;let E=r-m;const x=[];for(const i of s){const l=i.length+1;if(E-l<0)break;E-=l,x.push(i)}const S=o-x.length,k=c(S);let g=[a,...x,k,u].join(`
`);for(;x.length>0&&g.length>r;){x.pop();const i=o-x.length,l=c(i);g=[a,...x,l,u].join(`
`)}return g}function ge(e,t,n,r){const a=(r.getInjectedEntries?r.getInjectedEntries(n):[]).includes(e.id);switch(e.schedule.type){case"first":return!a;case"always":return!0;case"interval":{const u=e.schedule.everyNTurns||3;return a?(t-1)%u===0:!0}default:return!1}}function ye(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const o=A(r);if(!o.includes("<BDS:RP>"))continue;const a=o.match(/Character Name:\s*(.*?)\n/);if(a&&a[1])return a[1].trim()}return null}function Se(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<BDS:SKILLS fingerprint="(.*?)">/);if(a&&a[1])return a[1]}return null}function we(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<BDS:MCP fingerprint="(.*?)">/);if(a&&a[1])return a[1]}return null}function be(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<BDS:PROJECT name="(.*?)">/);if(a&&a[1])return a[1]}return null}function B(e){let t=String(e||"");return t=t.replace(/<Deepsick>([\s\S]*?)<\/Deepsick>/gi,(n,r)=>r.includes("[BDS:AUTO]")||r.includes("[BDS:DEEP_RESEARCH]")||/<BDS:memory_calls[\s>]/i.test(r)?n:""),t=t.replace(/<BDS:SKILLS>[\s\S]*?<\/BDS:SKILLS>/gi,""),t=t.replace(/<BDS:memory_calls[^>]*>[\s\S]*?<\/BDS:memory_calls>/gi,""),t=t.replace(/<BDS:RP>[\s\S]*?<\/BDS:RP>/gi,""),t=t.replace(/<BDS:PROJECT[^>]*>[\s\S]*?<\/BDS:PROJECT>/gi,""),t=t.replace(/<BDS:PROJECT_CONTEXT>[\s\S]*?<\/BDS:PROJECT_CONTEXT>/gi,""),t.trim()}function Te(){try{const e=document.querySelector("._46a12ab");if(!e)return null;const t=(e.textContent||"").toLowerCase().trim();return t.includes("vision")?"vision":t.includes("expert")||t.includes("reasoner")?"expert":t.includes("deepthink")||t.includes("deep think")||t.includes("r1")?"deepthink":t.includes("instant")||t.includes("chat")||t.includes("flash")?"instant":null}catch{return null}}function Ee(e){const t=e&&e.config&&e.config.deepCode;if(!t||!t.enabled)return"";const n=t.manualPath||t.activeDirectory||"active directory",r=t.fileTree?`
...
- Never emit BDS:HARNESS_TASK mid-explanation. It is always the final action
  of a turn.
</Deepsick>`}function ke(e){const t=e&&e.config&&e.config.deepCode,n=t&&t.pendingReport;if(!n||!n.report||!n.report.trim())return"";const r=n.cwd?` cwd="${n.cwd}"`:"",o=n.sessionId?` sessionId="${n.sessionId}"`:"";return`<Deepsick>
